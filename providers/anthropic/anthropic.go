				...
				if msg.Role == fantasy.MessageRoleUser {
					...
					for i, part := range msg.Content {
						...
						if result.ProviderExecuted {
							// Reconstruct web_search_tool_result and append
							// it to the previous assistant message, since
							// Anthropic expects it as an assistant-role block.
							if len(messages) == 0 {
								continue
							}
							prevMsg := &messages[len(messages)-1]
							...
							prevMsg.Content = append(prevMsg.Content, buildWebSearchToolResultBlock(result.ToolCallID, searchMeta))
							continue
						}
